      ...
      <div className={styles.topics}>
        {
          projectGithubData.repositoryTopics.edges.map((item, index) => {
            return (<a key={index} href={`https://github.com/topics/${item.node.topic.name}`} target="_blank" rel="noreferrer nofollow" title={item.node.topic.name}><p>{item.node.topic.name}</p></a>);
          })
        }
      </div>
